...
export default function DiceRollModal({ roll, die1, die2, onClose, tileInstruction }: DiceRollModalProps) {
      ...
      <div className="bg-gradient-to-br from-purple-900 to-pink-900 rounded-2xl p-8 max-w-md w-full border-2 border-purple-500/50 shadow-2xl text-center">
          ...
          {die1 && die2 ? (
            <div className="flex justify-center gap-4 mb-4">
              <div className="bg-white/20 rounded-xl p-6 border-2 border-white/30">
                ...
                <div className="text-3xl font-bold text-white">{isAnimating ? displayRoll : die1}</div>
              </div>
              <div className="text-4xl text-white flex items-center">+</div>
              <div className="bg-white/20 rounded-xl p-6 border-2 border-white/30">
                <div className="text-4xl mb-2">🎲</div>
                <div className="text-3xl font-bold text-white">{isAnimating ? displayRoll : die2}</div>
              </div>
            </div>
          ) : null}
          
          <div className="bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl p-8 border-2 border-white/30">
            <div className="text-sm text-purple-200 mb-2 uppercase tracking-wide">Total Roll</div>
            <div className={`text-7xl font-bold text-white transition-all ${isAnimating ? 'animate-pulse' : ''}`}>
              {displayRoll}
            </div>
        ...
        {tileInstruction && !isAnimating && (
          <div className="mb-4 bg-white/10 rounded-xl p-4 border border-white/20">
            <div className="text-sm text-purple-200 mb-1">Tile Instruction:</div>
